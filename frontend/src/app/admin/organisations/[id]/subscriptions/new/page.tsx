...
export default function AdminNewSubscriptionPage() {
  ...
  const onSubmit = async (data: FormData) => {
    setApiError(null);
    try {
      const res = await subscriptionsApi.create(orgId, {
        ...data,
        description: data.description || undefined,
      });
      router.push(`/admin/organisations/${orgId}/subscriptions/${res.data.data.id}`);
    } catch (err: unknown) {
      const msg =
  ...
  return (
    <>
      <PageHeader
        ...
        description="Create a new subscription type for this organisation."
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
          ...
          <div className="flex items-center gap-3 pt-2">
            <button
              ...
              className="px-5 py-2 bg-primary-600 text-white rounded-lg text-sm font-semibold hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {isSubmitting ? 'Creating…' : 'Create subscription'}
            </button>
            <button
              type="button"
              onClick={() => router.push(`/admin/organisations/${orgId}/subscriptions`)}
