...
export default function AdminSubscriptionDetailPage() {
  const { id: orgId, subId } = useParams<{ id: string; subId: string }>();
  ...
  useEffect(() => {
    ...
    subscriptionsApi.get(orgId, subId)
      .then(({ data }) => {
        setSub(data.data);
  ...
  const handleDeleteOption = async (optionId: string) => {
    ...
    await subscriptionsApi.deletePriceOption(orgId, subId, optionId).catch(() => {});
  ...
  const onAddOption = async (data: PriceOptionForm) => {
    ...
    try {
      const res = await subscriptionsApi.addPriceOption(orgId, subId, data);
  ...
  return (
    <>
      <PageHeader
        ...
        backHref={`/admin/organisations/${orgId}/subscriptions`}
            ...
            <form onSubmit={handleSubmit(onAddOption)} className="space-y-4">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="px-4 py-1.5 bg-primary-600 text-white rounded-lg text-sm font-semibold hover:bg-primary-700 disabled:opacity-50 transition-colors"
                >
      ...
      <div className="bg-white rounded-xl border border-red-200 p-6">
        ...
        <button
          onClick={async () => {
            ...
            await subscriptionsApi.delete(orgId, subId).catch(() => {});
            router.push(`/admin/organisations/${orgId}/subscriptions`);
          }}
