...
function NavLink({ item, orgSlug }: { item: NavItem; orgSlug: string }) {
  const pathname = usePathname();
  const href = item.href(orgSlug);
  const dashboardHref = `/${orgSlug}/portal`;
  const active =
    href === dashboardHref
      ? pathname === href
      ...
      <aside className="w-60 shrink-0 bg-white border-r border-gray-200 flex flex-col sticky top-0 h-screen">
        ...
        {orgSlug && (
          <div className="px-5 py-2 bg-violet-50 border-b border-violet-100">
            <p className="text-xs text-violet-600 font-medium truncate">{orgSlug}</p>
          </div>
        )}
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          ...
          {portalNav.map((item) => (
            <NavLink key={item.href(orgSlug)} item={item} orgSlug={orgSlug} />
          ))}
        </nav>
